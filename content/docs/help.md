...
- Try `https://quark.local` (or `http://quark.home.local` if that is what your install prints)
- On the iOS or Android app, open **Add Quark** and see whether your device appears in the discovered list
...
remote access so you won't need a third-party VPN to reach it from outside your home. That feature is not live yet.)

### I'm having performance issues

1. Open **System** and check the **Health** tab — high disk usage or high temperatures can cause slowdowns
2. In Settings, use **Export performance metrics** — this saves recent health data to a `metrics.sqlite` file
...
In Settings, open **Account and data**. Delete account is available to every login; Reset this Quark is admin-only.
Both ask for your account password. Deleting a login is not the same as wiping the box — read the confirmations.
